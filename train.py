...
from loss import loss_dehaze, loss_depth
...
parser.add_argument('--data_dir', default='/media/d/zs/DDHAZE/data', type=str, help='path to dataset')
...
def train(train_loader,
		  network, DEPTH_NET,
		  criterion_depth, criterion_dehaze,
	...
	for batch in train_loader:
		...
		with autocast(args.no_autocast):
			...
			t_d1, t_d2, t_d3 = deep_estimate_net(target_img.detach())
			o_d1, o_d2, o_d3 = deep_estimate_net(dehaze_output_img.detach())
			dehaze_output_img_2_depth_img = DEPTH_NET(dehaze_output_img.detach())
			real_img_2_depth_img = depth_decoder(encoder(target_img.detach()))
			real_img_2_depth_img = real_img_2_depth_img[("disp", 0)]
			loss_consis, deep_loss, loss_CR, loss_dehaze_total, loss_consis2 = criterion_dehaze(dehaze_output_img, target_img,
																				  dehaze_output_img_2_depth_img, real_img_2_depth_img,
																				  t_d1, t_d2, t_d3, o_d1, o_d2, o_d3,source_img)
			loss_depth2, loss_depth1, loss_total_depth = criterion_depth(dehaze_output_img, target_img, dehaze_output_img_2_depth_img, real_img_2_depth_img)

		Dehaze_loss.update(loss_dehaze_total.item())
		optimizer_dehaze.zero_grad()
		scaler.scale(loss_dehaze_total).backward(retain_graph=True)
		scaler.step(optimizer_dehaze)
		scaler.update()

		Depth_loss.update(loss_total_depth.item())
		optimizer_depth.zero_grad()
		scaler.scale(loss_total_depth).backward()
...
if __name__ == '__main__':
	...
	criterion_dehaze = loss_dehaze.Dehaze_loss()
	criterion_depth = loss_depth.Depth_loss()
	...
	if not os.path.exists(os.path.join(save_dir, args.model+'.pth')):
		...
		for epoch in tqdm(range(setting['epochs'] + 1)):
			...
			dehaze_loss, depth_loss = train(train_loader,
											network, DEPTH_NET,
											criterion_depth, criterion_dehaze,
											optimizer_dehaze, optimizer_depth,
											scaler
											)
